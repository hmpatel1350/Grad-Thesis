...
run = wandb.init(
    ...
    name = "Increased Iterations and Path"
)
...
def getH(batch_idx):
    return copy.deepcopy(H_cache[batch_idx])

for epoch in range(config['epochs']):
    iterations = 1 + (epoch * iterations_max) // (config['epochs'])
    paths = 1 + (epoch * path_length) // (config['epochs'])
    # Precompute for the current epoch
    for batch_idx, (P0, P1) in enumerate(dataloader):
        P0 = copy.deepcopy(P0[:, 0:paths, :])
        P1 = copy.deepcopy(P0[:, 0:paths, :])
        ...
        if epoch == 0:
            H_initial = torch.zeros(P0.shape[0], iterations, paths, h_size).to(accelerator.device)
            H_cache.append(H_initial)

        H_old = getH(batch_idx)

        zeros_j = torch.zeros(H_old.shape[0], H_old.shape[1], 1, H_old.shape[3]).to(accelerator.device)
        zeros_i = torch.zeros(H_old.shape[0], 1, H_old.shape[2], H_old.shape[3]).to(accelerator.device)

        H_i = torch.cat((zeros_i, H_old), dim=1)
        if(epoch % (config['epochs'] / iterations_max) != 0 or epoch == 0):
            H_i = H_i[:, :-1, :, :]
        if (epoch % (config['epochs'] / path_length) == 0 and epoch != 0):
            H_i = torch.cat((H_i, torch.zeros(H_i.shape[0], H_i.shape[1], 1, H_i.shape[3]).to(accelerator.device)), dim=2)

        H_j = torch.cat((zeros_j, H_old), dim=2)
        if(epoch % (config['epochs'] / path_length) !=0 or epoch == 0):
            H_j = H_j[:, :, :-1, :]

        if(epoch % (config['epochs'] / iterations_max) == 0 and epoch != 0):
            H_j = torch.cat((H_j, torch.zeros(H_j.shape[0], 1, H_j.shape[2], H_j.shape[3]).to(accelerator.device)), dim=1)
        ...
        H_new = h.reshape(P0.shape[0], iterations, paths, h_size).detach()
        H_cache[batch_idx] = copy.deepcopy(H_new)
...
P1 = torch.zeros_like(P0).to(accelerator.device)
H = [[None for _ in range(path_length)] for _ in range(iterations_max)]
for j in range(path_length):
    for i in range(iterations_max):
        if i == 0:
            h_i = torch.zeros(P0.shape[0], h_size).to(accelerator.device)
        else:
            h_i = H[i-1][j]
        if j == 0:
            h_j = torch.zeros(P0.shape[0], h_size).to(accelerator.device)
        else:
            h_j = H[i][j-1]
...
plt.title(f'Time to run: {elapsed_time} seconds')
